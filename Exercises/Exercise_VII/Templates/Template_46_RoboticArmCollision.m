...
% *************************************************************************
% ToDo:                                                                  
% Implement the computation of the collision impulse 'Lambda' (replace the
% place holder of zeros) and use it to compute the values for q_PLUS and
% d_q_PLUS,  the joint angles and joint velocities right after the
% collision.  As a placeholder, they are set to the values right before the
% collision, which is wrong, as it ignores the collision process.
% *************************************************************************
...
function [value, isterminal, direction] = wireStretched(~, y_)
    ...
    % *************************************************************************
    % ToDo:                                                                  
    % Implement a function for 'value' that replaces the '-1'.  The function
    % should be, such that 'value' goes from negative to positive with a
    % zero-crossing exactly when the wire is fully stretched. 
    % *************************************************************************
...
function [J_lambda, sigma_lambda] = GetConstraintTerms()
    ...
    % *************************************************************************
    % ToDo:             
    % By expanding the terms for sigma_lambda, we can implement a Baumgarte
    % stabilization.  Add the required terms to sigma lambda.  Use the
    % constants 'P_Baumgarte' and 'D_Baumgarte' in your code.
    % *************************************************************************
